...
@NamedQueries({
		...
		@NamedQuery(name = "PlanEntity.findPlan", query = "SELECT p FROM PlanEntity p WHERE LOWER(p.nombrePlan) LIKE LOWER(CONCAT('%', :nombrePlan, '%')) AND LOWER(p.descripPlan) LIKE LOWER(CONCAT('%', :descripPlan, '%')) AND p.fechaPlan LIKE CONCAT ('%', :fechaPlan, '%') AND p.objetivo LIKE CONCAT('%', :objetivo, '%') AND p.borradoPlan=0"),
		@NamedQuery(name = "PlanEntity.numberFindPlan", query = "SELECT COUNT(p) FROM PlanEntity p WHERE LOWER(p.nombrePlan) LIKE LOWER(CONCAT('%', :nombrePlan, '%')) AND LOWER(p.descripPlan) LIKE LOWER(CONCAT('%', :descripPlan, '%')) AND p.fechaPlan LIKE CONCAT ('%', :fechaPlan, '%') AND p.borradoPlan=0"),
		@NamedQuery(name = "PlanEntity.numberFindPlanWithObjetivo", query = "SELECT COUNT(p) FROM PlanEntity p WHERE LOWER(p.nombrePlan) LIKE LOWER(CONCAT('%', :nombrePlan, '%')) AND LOWER(p.descripPlan) LIKE LOWER(CONCAT('%', :descripPlan, '%')) AND p.fechaPlan LIKE CONCAT ('%', :fechaPlan, '%') AND p.objetivo LIKE CONCAT('%', :objetivo, '%') AND p.borradoPlan=0"),
...
public class PlanEntity {
	...
	@ManyToOne
	@JoinColumn(name = "id_objetivo", referencedColumnName = "id_objetivo", updatable = true)
