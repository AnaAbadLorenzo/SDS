...
public class JWToken implements Serializable {
	...
	public String generateToken(final Authentication auth) {
		...
		authorities = auth.getAuthorities().stream().map(GrantedAuthority::getAuthority)
				.collect(Collectors.joining(","));

		final String jwt = Jwts.builder().setSubject(auth.getName()).claim(AUTHORITIES, authorities)
	...
	private Boolean hasExpirate(final String token) {
	...
	public UsernamePasswordAuthenticationToken getAuthenticationToken(final String token, final Authentication auth,
			final UserDetails userDetails) {
		final JwtParser parser = Jwts.parser().setSigningKey(SIGN);
